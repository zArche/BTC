...
def api_key_get(params, request_path):
    ...
    params.update({'AccessKeyId': Config.ACCESS_KEY,
    ...
    params['Signature'] = createSign(params, method, host_name, request_path, Config.SECRET_KEY)
...
def api_key_post(params, request_path):
    method = 'POST'
    timestamp = datetime.datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%S')
    params_to_sign = {'AccessKeyId': Config.ACCESS_KEY,
                      'SignatureMethod': 'HmacSHA256',
                      'SignatureVersion': '2',
                      'Timestamp': timestamp}
    ...
    host_name = urlparse.urlparse(host_url).hostname
    host_name = host_name.lower()
    params_to_sign['Signature'] = createSign(params_to_sign, method, host_name, request_path, Config.SECRET_KEY)
